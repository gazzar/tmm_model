...
def rayleigh_map(p, imap, angle, maia_d):
    ...
    helpers.zero_outside_mask(imap, mask=p.el_maps['matrix'])
    ...
    for col in range(maia_d.cols):
        ...
        delta_theta_yx = maia_d.yx_angles_radian(row, col)
        delta_theta_x = delta_theta_yx[1]*180/np.pi
        ...
        # Get the Rayleigh scattering cross section, sigma_R
        # sigma_R = xraylib(energy, angle_to_Maia_element)
        sigma_R = 1.0
    
        # multiply everything by sigma_R
        imap_rm *= sigma_R
...
if __name__ == '__main__':
    ...
    maia_d = maia.Maia()
    r_map = rayleigh_map(p, i_map, angle, maia_d)
